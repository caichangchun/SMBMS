...
$.post("jsp/getTopnProduct",{"date":date},function(result){

    $.each(result,function (i) {
        topnproduct_data.push({index:i+1,source:this.productname,data:[{'value':this.price}]})
    ...
    topnproduct_data.forEach(function(d, index) {
        ...
        $tr.append($index);
        //source
        var $source = $('<td class="g2-table-column" ">' + d.source + '</td>');
        $tr.append($source);
        //data
...
/*每日销售额组成饼状图*/
$.post("jsp/getDayProductPercent",{date:date},function(result){
    //console.log(result);
    var ProductPercent_data=[];
    var sum=0;
    $.each(result,function (i) {
        sum+=this.dayproductpercent_price;
    });
    $.each(result,function (i) {
        ProductPercent_data.push({type:this.dayproductpercent_name,value:this.dayproductpercent_price/sum});
    });
