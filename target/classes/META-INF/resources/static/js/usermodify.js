...
$(function(){
	...
	saveBtn = $("#save");
	backBtn = $("#back");
	modifyusercode=$("#modifyusercode").val();
	...
	params={"usercode":modifyusercode};
	$.post("/jsp/userview",params,function(result){
         $("#username").val(result.username);
//         if(result.gender==1) sex="女";
//         else sex="男";
//         $("#gender").val(sex);
         ...
         //userRole.options[result.userrole].selected=true;
         //$("#userrole  option[value='1'] ").attr("selected",true);
	});
	...
	$.ajax({
    		...
    		success:function(data){//data：返回数据（json对象）
    			if(data != null){
    				...
    				if(this.id==roleid){
    				options += "<option selected=\"selected\" value=\""+this.id+"\">"+this.rolename+"</option>";}
    				else{
